...
Ahora se calculará la cuantía individual de cada uno de los siniestros en cada año, a partir de una distribución Log Normal, con $\hat{\mu} = 13,64315$ y $\hat{\sigma_x} = 0,4692848$. Dichas estimaciones son las *estimaciones máximo verosímiles* de esta distribución y se obtienen de la siguiente manera:

$$\hat{\mu} = \frac{1}{n}\sum_{i=1}^n log(x_i) = 13,64315 \ ,\ \hat{\sigma}= \frac{1}{n}\sum_{i=1}^n (log(x_i)-\hat{\mu})^2= 0,4692848$$
...
$$ y_0 = 3.249.248.925 + 2,326\times61.547.862 = 3.392.409.252 $$
...
Nuevamente, el parámetro de la distribución Poisson ($E(N) = \lambda$, donde N es la cantidad de siniestros por póliza) será $\hat{\lambda} = 0.135$.
...
Ahora se calculará la cuantía individual de cada uno de los siniestros en cada año, a partir de una distribución de Weibull, con $\hat{\alpha} = 1.794$ y $\hat{\beta} = 1052668.59$. Dicha estimación de los parámetros se realizó mediante el *método de máxima verosimilitud*(el cálculo mencionado no se realiza con una formula simple, sino que debe recurrirse a métodos numéricos):
...
$$ y_0 = 3.237.807.477 + 2,326\times63.682.695 = 3.385.933.426 $$
...
Al igual que en los casos anteriores, se simulará la distribución de la cantidad de siniestros mediante la distribución Poisson con parámetro $\hat{\lambda} = 0.135$.
...
if(FALSE){for(i in 1:100000){ 
siniestros3[i] <- sum(rpois(n = 25615, lambda = 0.135))
...
Obteniendo una media en la cantidad de siniestros igual a 3458 siniestros, y un desvío estandar de 59 siniestros.
...
$$\hat{\beta}  = \frac{S^2_x}{\overline{X}} = 346274.2 \ , \ \hat{\alpha} = \frac{\overline{X}}{\hat{\beta}} = 2.728781$$
...
if(FALSE){for(i in 1:length(siniestros3)) {
  simulaciones3[[i]] <- rgamma(siniestros3[i], shape = 2.728781, scale = 346274.2)
}}
...
Nuevamente se observa una distribución muy simetrica, esto puede deberse a la elevada cantidad de simulaciones realizadas (100.000), donde observamos un coeficiente de asimetría muestral igual a $0.0309$.

Por lo tanto, y al igual que los casos anteriores, el Margen de Solvencia Mínimo se calculará mediante una aproximación Normal de las cuantías totales con $\mu = 3.267.268.560$ y $\sigma = 65.025.438$.

#### Margen de Solvencia Mínimo

Una vez más, se utilizarán recargos de seguridad del 1% y 3%.

Para que la probabilidad de solvencia sea del 99% (O la probabilidad de Ruina Técnica sea del 1%), se necesita conocer el valor de la Normal Estandar que acumula el 99% de probabilidad.

$$ Z \sim  N(0,1)  \rightarrow P(Z < z_0) = 0,99 \rightarrow z_0 = 2,326$$

-   Para un *recargo de seguridad (R) del 1%*:

$$E(Y) = 3.267.268.560 , \ SD(Y) = 65.025.438 , \ R = E(Y) \times 0,01 = 32.672.686 , \ PR = E(Y) + R = 3.299.941.246$$

Entonces el monto total a cobrar al cliente para garantizar una solvencia del 99% es:

$$ y_0 = 3.267.268.560 + 2,326 \times 65.025.438 = 3.418.517.729 $$

Y el Margen de Solvencia Mínimo resulta:

$$MSM = y_0 - PR = 3.418.517.729 - 3.299.941.246 = 118.576.483$$

-   Para un *recargo de seguridad (R) del 3%*:

$$E(Y) = 3.267.268.560 , \ SD(Y) = 65.025.438 , \ R = E(Y) \times 0,03 = 98.018.057 , \ PR = E(Y) + R = 3.365.286.617$$
...
$$MSM = y_0 - PR = 3.418.517.729 - 3.365.286.617 = 53.231.112$$
...
x <- seq(0, 12000000, length.out = 100000)
...
# Weibull
dens_wb <- dweibull(x, shape = 1.79472, scale = 1052668.59354)
df_wb <- data.frame(x = x, y = dens_wb, distribucion = "Weibull")

# Gamma
dens_gm <- dgamma(x, shape = 2.728781, scale = 346274.2)
...
tabla_msm <- tribble(
  ~Distribución,     ~`MSM (1%)`,        ~`MSM (3%)`,
  "Log-normal",      110667838,          45682859,
  "Weibull",         115747874,          50991725,
  "Gamma",           118576483,          53231112
)
